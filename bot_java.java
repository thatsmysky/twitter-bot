...
//Lucy Kull
    ...
    //list of verbs
    String[] verb= new String[]{"accept","ache","acknowledge","act","add","admire","admit","admonish",
            "advise","adopt","affirm",
            ...
            "bar","bare","bargain","bark", "barrage","barter","baste","bat","bathe","battle","bawl","be","beam","bear","beat","become",
    ...
    //list of nouns
    String[] noun= new String[]{"account","achiever","acoustics","act","action","activity","actor","addition","adjustment",
    ...
    //list of adjectives
    String[] adjective= new String[]{"abandoned","able","absolute","adorable","adventurous","academic","acceptable","acclaimed",
    ...
    String[] st_character= new String[]{"Captain Kirk","Commander Spock","James T Kirk","Spock","Bones","Leonard McCoy","Doctor McCoy",
    ...
    String[] rand_character= new String[]{"Jefferson","Burr","Hamilton","Aaron Burr","Alexander Hamilton","Thomas Jefferson",
    ...
    String[] firefly_character= new String[]{"Malcolm","Mal","Malcolm Reynolds","Zoe","Zoe Washburne","Kaylee","Kaylee Fry","Jayne",
    ...
    String[] st_series= new String[]{"Star Trek","Star Trek: The Original Series"};


    //list of plot-building events to make the tweet amusing
    String[] st_plot= new String[]{"contracts a disease that makes them speak exclusively in limericks","technobabels",
    ...
    String[] st_solution= new String[]{"has to bargain for a pair of ceremonial underpants","has to kill it","gets drunk","dances it off",
    ...
    void log(String message) {
        ...
        with open (os.path.join(path, logfile_name), 'a+') as f : t = strftime("%d %b %Y %H:%M:%S", gmtime());
        f.write("\n" + t + " " + message);
    }
    ...
    {
        if (syntax_choice == 1) {
            //we want to make a Star Trek Episode
            String tweet_text = create_tweet(st_series, st_character, st_plot, st_solution, verb, noun, syntax_choice);
            tweet(tweet_text);
        }

        else if (syntax_choice ==2) {
            //we want to make a lame news report
            String tweet_text = create_tweet(st_series, rand_character, st_plot, st_solution, verb, noun, syntax_choice);
            tweet(tweet_text);
        }

        else if (syntax_choice ==3) {
            // we want to become a self-aware twitterbot
            String tweet_text = create_tweet(st_series, rand_character, st_plot, st_solution, verb, noun, syntax_choice);
            tweet(tweet_text);
        }

        else if (syntax_choice ==4) {
            //we want a random character to do something weird
            String tweet_text = create_tweet(st_series, rand_character, st_plot, st_solution, verb, noun, syntax_choice);
            tweet(tweet_text);
        }
    }

    String create_tweet(String[] series, String[] character, String[] plot, String[] solution, String[] verbs, String[] nouns, int syntax) {
        ...
        if (syntax == 1) {
            ...
            //we're doing some randomization
            int series_choice = (int) (Math.random() * series.length);

            //variable to choose which series to tweet
            int character_choice = (int) (Math.random() * character.length);

            //variable to choose which character to tweet
            int plot_choice = (int) (Math.random() * st_plot.length);

            //variable to choose which plot to tweet
            int solution_choice = (int) (Math.random() * st_solution.length);

            //variable to choose which solution to tweet
            String text = "In this episode of " + series[series_choice] + " " + character[character_choice] + " " + plot[plot_choice] + " and " + solution[solution_choice];
            return text;
        }
        ...
        if (syntax == 2) {
            ...
            //we're doing some randomization
            int character_choice1 = (int) (Math.random() * character.length);

            //variable to choose first character to tweet
            int character_choice2 = (int) (Math.random() * character.length);

            //variable to choose second character to tweet
            int plot_choice = (int) (Math.random() * st_plot.length);
            ...
            //variable to choose which solution to tweet
            String text = "Today, " + character[character_choice1] + " " + plot[plot_choice] + ", " + character[character_choice2] + " " + solution[solution_choice];
            return text;
        }
        ...
        if (syntax == 3) {
            ...
            //we 're doing some randomization
            int noun_choice1 = (int) (Math.random() * nouns.length);

            //variable to choose first noun to tweet
            int noun_choice2 = (int) (Math.random() * nouns.length);

            //variable to choose second noun to tweet
            int verb_choice = (int) (Math.random() * verb.length);

            //variable to choose which verb to tweet
            String text = "I, a robot, made this tweet. #" + nouns[noun_choice1] + " #" + nouns[noun_choice2] + " #" + verbs[verb_choice];
            return text;
        }
        ...
        if (syntax == 4) {
            ...
            //variable to choose first noun to tweet
            int noun_choice = (int) (Math.random() * nouns.length);

            //variable to choose second noun to tweet
            int verb_choice = (int) (Math.random() * verb.length);

            //variable to choose which verb to tweet
            String text = character[character_choice] + " will " + verbs[verb_choice] + " the " + nouns[noun_choice];
            return text;
        }
        return "";
    }
